...
const HtmlIntermediate = () => {
  ...
  <div className="max-w-lg mx-auto overflow-y-auto pt-10">
    <h1 className="text-3xl font-bold mb-6 text-center">HTML Intermediate Quiz</h1>
          ...
          {q.options && q.options.map((option, optionIndex) => (
            <li key={optionIndex} className="mb-2">
              <label className="flex items-center">
                <input type="radio" name={`q${index}`} value={option} className="mr-2" />
                {option}
              </label>
            </li>
          ))}
  ...
  <div className="flex justify-center mt-8 pb-8">
    ...
    <button className='bg-green-400 px-5 border border-green-100 rounded-2xl py-3 mx-4' onClick={SubmitEvent}>Submit</button>
  </div>
</QuizLayout>
  );
};
